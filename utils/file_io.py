...
def save_uploaded_files(uploaded_files: Iterable, target_dir: Path) -> List[Path]:
    """Save uplaoded files (Streamlit-like) and return local paths."""
    try:
        ...
        for uf in uploaded_files:
            ...
            if ext not in SUPPORTED_EXTENSIONS:
                log.warning("Uncensored file skipped", filename=name)
                continue
        ...
        raise DocumentRAGException("Failed to save uploaded files", e) from e